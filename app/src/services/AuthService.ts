...
export const signIn = async (email: string, password: string): Promise<AuthResponse> => {
    try {
        ...
        if (error.response) {
            if (error.response.data && error.response.data.error === "Utilisateur non trouvé dans Clerk") {
                return {
                    success: false,
                    error: "Votre compte n'existe plus ou a été désactivé. Veuillez contacter le support."
                };
            }
        ...
        if (error.response) {
            ...
            if (error.response.status === 404) {
                return {
                    ...
                    details: error.response.data.details || "L'utilisateur n'existe pas dans Clerk"
                };
            }
