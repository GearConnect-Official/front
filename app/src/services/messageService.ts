...
export class MessageService {
  ...
  static readonly INFO = {
    ...
    SUPPORT_CONTACT: {
      type: MessageType.INFO,
      message:
        "Veuillez contacter support@gearconnect.com pour obtenir de l'aide",
      duration: 6000,
    } as MessageConfig,
    ...
    TERMS_CONDITIONS: {
      type: MessageType.INFO,
      message:
        "Les conditions d'utilisation complètes seront disponibles dans la prochaine mise à jour",
      duration: 5000,
    } as MessageConfig,
    ...
    TWO_FA_REQUIRED: {
      type: MessageType.INFO,
      message:
        "L'authentification à deux facteurs est requise pour terminer la réinitialisation du mot de passe",
      duration: 6000,
    } as MessageConfig,
  };
  ...
  /**
   * Crée une conversation entre utilisateurs
   * @param participantIds array d'IDs utilisateurs (obligatoire)
   * @param name nom du groupe (optionnel)
   * @param isGroup booléen pour groupe (optionnel)
   */
    ...
    try {
      const response = await axios.post(
        `${API_BASE_URL}/messaging/conversations`,
        {
          participantIds,
          ...(name ? { name } : {}),
          ...(typeof isGroup !== "undefined" ? { isGroup } : {}),
        }
      );
