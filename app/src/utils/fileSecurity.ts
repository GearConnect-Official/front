...
export const validateFileSafety = (
  ...
  if (!extension) {
    // File has no extension - could be dangerous
    if (!mimeType) {
      return {
        isValid: false,
    ...
    // If we have MIME type, continue to MIME validation
...
export const getAllowedFileTypes = (): string[] => {
  return [
    ...
    'image/*',
    // Archives (with caution - could contain dangerous files, but user choice)
    'application/zip',
